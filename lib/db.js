...
async function ensureSchema() {
    ...
    await query(`CREATE TABLE IF NOT EXISTS ${t('assistant_task_message')} (
        ...
        role         ENUM('user','assistant','estratega') NOT NULL,
        content      MEDIUMTEXT   NOT NULL,
        created_at   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
        PRIMARY KEY (id),
        KEY idx_msg_task (task_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`);
    // 'estratega': notas TÉCNICAS de coordinación (supervisión + acciones). Van a la
    // columna del Estratega, NO al chat (que es solo negocio: "Trabajando…" y preguntas).
    await query(`ALTER TABLE ${t('assistant_task_message')} MODIFY role ENUM('user','assistant','estratega') NOT NULL`).catch(() => {});
