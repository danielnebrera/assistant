...
// ║                                                                            ║
// ║  Mismo modelo que el resto de la plataforma (ver dev/shared/db.js): UN     ║
// ║  pool a la BD de empresa (`factory`) y un PREFIJO de entorno por tabla     ║
// ║  (dev_ / test_ / pro_). El assistant POSEE sus tablas `assistant_*` y no   ║
// ║  toca las de ningún módulo (Regla de Oro).                                 ║
// ║                                                                            ║
// ║  Config 100% por entorno (.env del assistant): DB_HOST/DB_USER/            ║
// ║  DB_PASSWORD/DB_DATABASE + ENV. Así el repo común no lleva nada del        ║
// ║  proyecto: cada despliegue apunta a su BD con su prefijo.                  ║
// ╚══════════════════════════════════════════════════════════════════════════╝
const mysql = require('mysql');

const ENV    = (process.env.ENV || 'dev').toLowerCase();   // dev | test | pro
const PREFIX = ENV + '_';
...
// t('assistant_task') -> 'dev_assistant_task'
...
module.exports = { pool, query, t, ensureSchema, ensureColumn, ENV, PREFIX };
