...
app.use(express.urlencoded({ extended: false }));   // para el form de login propio
...
function gate(req, res, next) {
    ...
    if (!user) {
        ...
        // Login PROPIO del assistant: valida contra la BD, así se puede entrar
        // aunque la `app` esté caída. Conserva el destino original en ?next.
        return res.redirect('/assistant/login?next=' + encodeURIComponent(req.originalUrl));
    }
...
// Login propio: SIN gate (si no, redirección infinita). Resuelve el acceso
// leyendo la BD directamente — operativo aunque la `app` no responda.
app.get('/assistant/login',  login.form);
app.post('/assistant/login', login.submit);
app.get('/assistant/logout', login.logout);
